...
export default function MentorProjects() {
  ...
  const fetchProjects = async () => {
    try {
      ...
      const response = await fetch(`/api/mentors/projects?${params}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleChangeStatus = async (projectId: string, newStatus: 'completed' | 'on-hold') => {
    try {
      const response = await fetch(`/api/mentors/projects/${projectId}/status`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        ...
        alert(`Project status updated to ${newStatus.replace('-', ' ').toUpperCase()}!`);
      } else {
        alert('Failed to update project status.');
      }
    } catch (error) {
      console.error('Error changing project status:', error);
      alert('An error occurred while changing project status.');
    }
  };
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex space-x-4">
              <button
                onClick={() => router.push('/mentors/projects/create')}
                  ...
                  <div className="flex justify-between items-start mb-4">
                      ...
                      {project.progressReport && project.progressReport.length > 0 && (
                        <div className="mt-2 text-sm text-blue-800 bg-blue-50 rounded p-2">
                          ...
                          <ul className="list-disc pl-5">
                            {project.progressReport.map((report, idx) => (
                              <li key={idx}>
                    ...
                    <div className="flex flex-col items-end space-y-2">
                      ...
                      {project.status !== 'completed' && (
                        <button
                          onClick={() => handleChangeStatus(project._id, 'completed')}
                      ...
                      {project.status !== 'on-hold' && (
                        <button
                          onClick={() => handleChangeStatus(project._id, 'on-hold')}
                          className="mt-1 bg-yellow-500 text-white px-3 py-1 rounded hover:bg-yellow-600 text-xs"
                        >
                          Mark as On Hold
                        </button>
                      )}
